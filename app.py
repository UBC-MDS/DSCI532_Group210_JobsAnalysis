...
app.layout = html.Div([
    ...
    html.Div([
        html.Div([
            ...
            dcc.Dropdown(
                id='job_name',
                options=[{'label': i, 'value': i} for i in data_wrangling.get_unique_job_names()],
...
@app.callback(dash.dependencies.Output('single-job-plot', 'srcDoc'), [dash.dependencies.Input('job_name', 'value')])
def update_single_job_plot(job_name):
    return callback_functions.update_job_name_by_gender(job_name)


@app.callback(dash.dependencies.Output('job-dominancy-plot', 'srcDoc'), [dash.dependencies.Input('dominancy_groups', 'value')])
